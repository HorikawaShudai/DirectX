...
void UpdataBullet(void)
{
	...
	for (nCntBullet = 0; nCntBullet < MAX_BULLET; nCntBullet++)
	{
		if (g_aBullet[nCntBullet].bUse == true)
		{
			...
			if (g_aBullet[nCntBullet].Length>20.0f)
			{
			...
			else if (ColissionWall(g_aBullet[nCntBullet].pos) == true)
			{
				SetExplosion(g_aBullet[nCntBullet].pos, D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f));
				FinishShadow(g_aBullet[nCntBullet].nIdxShadow);
				g_aBullet[nCntBullet].bUse = false;
			...
			SetEffect(D3DXVECTOR3(g_aBullet[nCntBullet].pos), D3DXCOLOR(0.0f, 0.0f, 1.0f, 1.0f), 20.0f, 10);
